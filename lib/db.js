...
async function buildSchema() {
  ...
  // Batch all student count checks in one query
  const studentCountRows = await sql(
    `select class, count(*)::int as n from students where class = any($1) group by class`,
    [defaultScheduleClasses.map((s) => s.name)]
  );
  const studentCountMap = Object.fromEntries(studentCountRows.map((r) => [r.class, r.n]));

  // Batch all discipline count checks in one query
  const discCountRows = await sql(
    `select class, count(*)::int as n from discipline_records where class = any($1) group by class`,
    [defaultScheduleClasses.map((s) => s.name)]
  );
  const discCountMap = Object.fromEntries(discCountRows.map((r) => [r.class, r.n]));

  for (const sc of defaultScheduleClasses) {
    const stList = demoStudentsByClass[sc.name] || [];
    const curStudentCount = studentCountMap[sc.name] || 0;
    ...
    if (curStudentCount < stList.length) {
      ...
      // Batch insert all students in one query
      const stParams = [];
      const stPlaceholders = stList.map(([no, zh, en_full, en_first, sex], i) => {
        const base = i * 6;
        stParams.push(sc.name, no, en_full, zh, en_first, sex);
        return `($${base + 1}, $${base + 2}, $${base + 3}, $${base + 4}, $${base + 5}, $${base + 6})`;
      });
      await sql(
        `insert into students (class, no, name, name_zh, name_en, sex) values ${stPlaceholders.join(',')}
         on conflict (class, no) do update set
           name = excluded.name, name_zh = excluded.name_zh,
           name_en = excluded.name_en, sex = excluded.sex`,
        stParams
      );
      ...
      // Batch insert marks in one query
      const markNos = [1, 2, 4, 7, 9, 12, 15, 18].filter((n) => n <= stList.length);
      const markParams = [];
      const markPlaceholders = markNos.map((stNo, i) => {
        markParams.push(demoSessionId, stNo);
        return `($${i * 2 + 1}, $${i * 2 + 2})`;
      });
      if (markPlaceholders.length) {
        await sql(
          `insert into marks (session_id, student_no) values ${markPlaceholders.join(',')}
           on conflict (session_id, student_no) do nothing`,
          markParams
        );
      }
...
export async function updateDisciplineRecord(id, note = '') {
  const rows = await sql(
