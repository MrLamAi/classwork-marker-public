...
async function buildSchema() {
  ...
  if (n === 0) {
    ...
    await sql('insert into classes (name, total, cols, seats) values ($1, $2, $3, $4)',
      ['1A', 32, 8, seats1A]);
...
async function getClass(name) {
  const rows = name
    ? await sql('select name, total, cols, seats from classes where name = $1', [name])
    : [];
...
export async function saveStudents(cls, list) {
  ...
  try {
    ...
    await client.query('delete from students where class = $1', [cls]);
    if (clean.length) {
      const cols = 5;
      ...
      await client.query(
        ...
        [cls, ...clean.flatMap((o) => [o.no, o.name || null, o.zh || null, o.en || null, o.sex || null])]
      );
    }
