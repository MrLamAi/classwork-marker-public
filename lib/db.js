...
async function buildSchema() {
  ...
  // 1. Seed demo students if students table is empty
  const [{ n: studentCount }] = await sql('select count(*)::int as n from students');
  if (studentCount === 0) {
    const seats1A = Array.from({ length: 32 }, (_, i) => (i < 20 ? String(i + 1) : '')).join(',');
    await sql('update classes set seats = $1, total = 32, cols = 8 where name = $2', [seats1A, '1A']);

    const seats2A = Array.from({ length: 30 }, (_, i) => (i < 10 ? String(i + 1) : '')).join(',');
    await sql('update classes set seats = $1, total = 30, cols = 6 where name = $2', [seats2A, '2A']);
    ...
    for (const [no, zh, en_full, en_first, sex] of demo1A) {
      await sql(
        `insert into students (class, no, name, name_zh, name_en, sex) values ($1, $2, $3, $4, $5, $6)
         on conflict (class, no) do nothing`,
        ['1A', no, en_full, zh, en_first, sex]
      );
    }
    ...
    for (const [no, zh, en_full, en_first, sex] of demo2A) {
      await sql(
        `insert into students (class, no, name, name_zh, name_en, sex) values ($1, $2, $3, $4, $5, $6)
         on conflict (class, no) do nothing`,
        ['2A', no, en_full, zh, en_first, sex]
      );
    }
    ...
    await sql(
      ...
      [demoSessionId, '1A', '課堂工作紙 1 (DEMO)']
    );

    for (const stNo of [1, 2, 4, 7, 9, 12, 15, 18]) {
      await sql(
    ...
    await sql(
      `insert into lesson_events (session_id, class, student_no, type, label, note) values
       ($1, '1A', 1, 'good_perf', '積極答問', '主動舉手回答問題'),
       ($1, '1A', 4, 'good_perf', '專注投入', '課堂表現認真'),
       ($1, '1A', 8, 'talking', '說話分心', '課堂討論過於喧嘩')
       on conflict do nothing`,
      [demoSessionId]
    );
  }

  // 2. Seed demo discipline records & past confirmed lessons if empty
  const [{ n: discCount }] = await sql('select count(*)::int as n from discipline_records');
  if (discCount === 0) {
    ...
    const dToday = calcDate(0);
    const dPrev1 = calcDate(2);
    const dPrev2 = calcDate(5);
    const dPrev3 = calcDate(8);
    ...
    await sql(
      ...
      ['1A', dPrev1, dPrev2, dPrev3]
    );

    await sql(
      `insert into discipline_records (class, student_no, record_date, record_time, type, label, note) values
       ('1A', 8, $1, '09:15', 'no_book', '欠帶課本', '未帶課本'),
       ('1A', 8, $1, '09:35', 'talking', '說話分心', '與同桌交頭接耳'),
       ('1A', 3, $1, '09:20', 'good_perf', '積極答問', '回答題目正確且詳盡'),
       ('1A', 5, $1, '09:40', 'talking', '說話分心', '課堂討論過於喧嘩'),
       ('1A', 8, $2, '14:20', 'no_hw', '欠交功課', '工作紙第3課未交'),
       ('1A', 8, $2, '14:25', 'no_book', '欠帶課本', '上一堂未帶課本'),
       ('1A', 8, $3, '10:05', 'no_book', '欠帶課本', '欠帶作業本'),
       ('1A', 8, $3, '10:30', 'sleeping', '課堂睡覺', '伏在課桌上'),
       ('1A', 3, $2, '11:15', 'good_perf', '積極答問', '主動上台板書解題'),
       ('1A', 1, $2, '11:30', 'good_perf', '積極答問', '踴躍回答'),
       ('1A', 4, $3, '10:20', 'good_perf', '專注投入', '認真筆記')
       on conflict do nothing`,
      [dToday, dPrev1, dPrev2]
    );
  }
}
